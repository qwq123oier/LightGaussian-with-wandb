    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                if args.use_wandb and WANDB_FOUND:
                    wandb.log({
                        ...
                        "loss/l1": Ll1.item(),
                        "loss/ema": ema_loss_for_log,
                        "iteration": iteration,
                        "gaussians/count": gaussians.get_xyz.shape[0]
                    }, step=iteration)
            ...
            if iteration in args.prune_iterations:
                ...
                gaussian_count_before = gaussians.get_xyz.shape[0]
                gaussian_list, imp_list = prune_list(gaussians, scene, pipe, background)
                i = args.prune_iterations.index(iteration)
                prune_ratio = (args.prune_decay**i) * args.prune_percent
                v_list = calculate_v_imp_score(gaussians, imp_list, args.v_pow)
                gaussians.prune_gaussians(prune_ratio, v_list)
                gaussian_count_after = gaussians.get_xyz.shape[0]
                
                # Log pruning to wandb
                if args.use_wandb and WANDB_FOUND:
                    wandb.log({
                        "pruning/iteration": iteration,
                        "pruning/ratio": prune_ratio,
                        "pruning/gaussians_before": gaussian_count_before,
                        "pruning/gaussians_after": gaussian_count_after,
                        "pruning/gaussians_removed": gaussian_count_before - gaussian_count_after,
                        "pruning/removal_percentage": (gaussian_count_before - gaussian_count_after) / gaussian_count_before * 100
                    }, step=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_project", type=str, default="QuadGaussian-Train", help="wandb项目名")
